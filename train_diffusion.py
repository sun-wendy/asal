...
class ResBlock(nn.Module):
    ...
    def __call__(self, x, t_emb):
        ...
        scale, shift = jnp.split(nn.Dense(x.shape[-1]*2)(t_emb), 2, axis=-1)
...
class UNet2D(nn.Module):
    ...
    def __call__(self, x, t):
        t_emb = nn.Dense(self.channels)(
                    nn.swish(
                      nn.Dense(self.channels)(
                        sinusoidal_emb(t))))
        hs = []
        h = nn.Conv(self.channels, (3,3), padding='SAME')(x)
        for _ in range(self.depth):
            h = ResBlock()(h, t_emb)
            hs.append(h)
            h = nn.avg_pool(h, (2,2))
        h = ResBlock()(h, t_emb)
        for h_skip in reversed(hs):
            h = jax.image.resize(h,
                (h.shape[0], h_skip.shape[1], h_skip.shape[2], h.shape[-1]),
                method='nearest')
...
# ─── Sinusoidal time embedding (force float32) ─────────────────────────────────
def sinusoidal_emb(t, dim=64):
    half = dim // 2
    # use jnp.log to stay in float32, and cast arange to float32
...
def main():
    ...
    model = UNet2D(channels=64, depth=3)
