...
def train_gpt(batch_size: int = 32, train_steps: int = 3000, eval_every: int = 200, img_size: int = 32, patches_per_dim: int = 2,
    ...
    block_size = (num_frames - 1) * num_tokens
    
    print(f"Training with batch size {batch_size}, train steps {train_steps}, eval every {eval_every}, img size {img_size}, patches per dim {patches_per_dim}, num frames {num_frames}, t_skip {t_skip}")
    ...
    with open(f"gpt_params_bsz{batch_size}_trainsteps{train_steps}_img{img_size}_patches{patches_per_dim}_frames{num_frames}_tskip{t_skip}.pkl", "wb") as f:
        pickle.dump(state.params, f)
    print(f"[Done] Model parameters saved to gpt_params_bsz{batch_size}_trainsteps{train_steps}_img{img_size}_patches{patches_per_dim}_frames{num_frames}_tskip{t_skip}.pkl")
    wandb.finish()
