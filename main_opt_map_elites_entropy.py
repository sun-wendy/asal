...
def main(args):
    ...
    # Define a coefficient for the entropy term (tune as needed)
    entropy_coef = 0.0005  # you may want to expose this as an argument

    def entropy_metric(z_seq):
        """
        Compute the differential entropy of the sequence of embeddings,
        assuming they are samples from a multivariate Gaussian.
        
        Args:
            z_seq: jnp.ndarray of shape (time, embed_dim)
        
        Returns:
            entropy: scalar representing the entropy.
        """
        time, d = z_seq.shape
        mu = jnp.mean(z_seq, axis=0, keepdims=True)
        centered = z_seq - mu
        eps = 1e-6
        cov = (centered.T @ centered) / (time - 1) + eps * jnp.eye(d)
        
        # Use slogdet for numerical stability
        sign, logdet = jnp.linalg.slogdet(cov)
        entropy = 0.5 * (d * jnp.log(2 * jnp.pi * jnp.e) + logdet)
        return entropy
    ...
    def performance(rng, x, prompt_idx):
        """
        Roll out x, compute prompt alignment from the final embedding,
        and compute an entropy metric based on the entire sequence of embeddings.
        The final score is a weighted combination of both.
        """
        rollout_data = rollout_jit(rng, x)
        z_seq = rollout_data['z']  # shape: (time, embed_dim)
        
        # Prompt alignment: use the final embedding for alignment.
        final_embedding = z_seq[-1]
        alignment_score = jnp.dot(final_embedding, z_prompts[prompt_idx])
        
        # Compute the entropy metric over the trajectory
        entropy_score = entropy_metric(z_seq)
        
        # Combine the alignment score and entropy metric.
        combined_score = alignment_score + entropy_coef * entropy_score
