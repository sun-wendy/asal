...
# ── compute normalized spatio-temporal entropy (unused for count prediction) ──
...
# ── compute patch temporal entropy (unused for count prediction) ──
...
# ── prepare and cache latents & counts ────────────────────────────────────────
def prepare_and_cache(
    ...
    if t_skip > 0:
        seqs = seqs[:, ::(t_skip+1)]
    # predict number of alive cells in final frame
    y = np.array([int(s[-1].sum()) for s in seqs], dtype=np.float32)
    inits = seqs[:, 0, ...]
...
# ── main: train surrogate MLP for cell count prediction ────────────────────────
if __name__ == '__main__':
    ...
    p.add_argument('--output_model', type=str, default='surrogate_mlp_count.joblib')
    args = p.parse_args()
    ...
    # prepare data
    train_cache = os.path.join(args.cache_dir, 'train_mid_count.npz')
    val_cache   = os.path.join(args.cache_dir, 'val_mid_count.npz')
    ...
    print(f"[y_train count] min={y_train.min():.0f} max={y_train.max():.0f}, mean={y_train.mean():.2f}, std={y_train.std():.2f}")
    print(f"[y_train_std] min={y_train_std.min():.3f} max={y_train_std.max():.3f}, mean={y_train_std.mean():.3f}, std={y_train_std.std():.3f}")
    print(f"[y_val count]   min={y_val.min():.0f} max={y_val.max():.0f}, mean={y_val.mean():.2f}, std={y_val.std():.2f}")
    ...
    plt.xlabel('Step')
    plt.ylabel('MSE')
    plt.title('Smoothed Training & Validation MSE (Cell Count)')
    plt.legend()
    plt.tight_layout()
    plt.savefig('surrogate_mse_smoothed_count.png')
