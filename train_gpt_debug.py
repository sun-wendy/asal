...
# ----------------- Evaluation Function -----------------
def evaluate_model(model, state, val_dataset: np.ndarray, img_size: int, grid_size: Tuple[int, int], step: int, t_skip: int = 0):
    """
    ...
    The new parameter t_skip (>=0) allows skipping frames in evaluation: only every (t_skip+1)th frame is used.
    """
    total_accuracy = 0.0
    ...
    # --- Apply t_skip for visualization as well ---
    if t_skip > 0:
...
def train_gpt(batch_size: int = 32, train_steps: int = 3000,
              ...
              t_skip: int = 0):   # <-- New hyperparameter added here.
    """
    ...
    t_skip: if greater than 0, only every (t_skip+1)th frame of the validation data is used during evaluation.
    """
    wandb.init(project="gol_world_model",
               name=f"gpt_patches{patches_per_dim}_batch{batch_size}_tskip{t_skip}",
    ...
    print(f"Training with patches_per_dim = {patches_per_dim}, grid_size = {grid_size}, token_dim = {token_dim}, "
          f"num_frames = {num_frames}, block_size = {block_size}, img_size = {img_size}, t_skip = {t_skip}")
    ...
    # The validation dataset is loaded if the file exists.
    ...
    for step in range(1, train_steps + 1):
        ...
        # Evaluate on the full validation dataset if present.
        if step % eval_every == 0 and val_dataset is not None:
            evaluate_model(model, state, val_dataset, img_size, grid_size, step, t_skip=t_skip)
